...
import coil.load
import com.example.myapplication.databinding.ItemFeedHeaderBinding
...
import com.example.myapplication.presentation.screen.feed.FeedItem
...
class FeedAdapter() : ListAdapter<FeedItem, RecyclerView.ViewHolder>(FeedDiff) {
    ...
    companion object {
        ...
        private const val VIEW_TYPE_FEED_HEADER = 3
    }
    ...
    override fun getItemViewType(position: Int): Int =
        when (getItem(position)) {
            ...
            is FeedItem.FeedHeaderRow -> VIEW_TYPE_FEED_HEADER
        }
    ...
    override fun onCreateViewHolder(
        ...
        return when (viewType) {
            ...
            VIEW_TYPE_MY_DRIVER -> MyDriverViewHolder(
            ...
            VIEW_TYPE_FEED_HEADER -> FeedHeaderViewHolder(
                ItemFeedHeaderBinding.inflate(inflater, parent, false)
            )
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val item = getItem(position)) {
            ...
            is FeedItem.MyDriverRow ->
                (holder as MyDriverViewHolder).bind(item.driver)
            is FeedItem.FeedHeaderRow -> Unit
        }
    }
    ...
    class MyDriverViewHolder(
        ...
        fun bind(item: Driver) {
            binding.ivDriver.load(item.favDriverImage) {
                crossfade(true)
            }
        }
    }
        ...
        override fun areItemsTheSame(old: FeedItem, new: FeedItem): Boolean {
            return when {
                ...
                old is FeedItem.MyDriverRow && new is FeedItem.MyDriverRow ->
                    old.driver.id == new.driver.id

                old is FeedItem.PostRow && new is FeedItem.PostRow ->
                    old.post.id == new.post.id

                old is FeedItem.FeedHeaderRow && new is FeedItem.FeedHeaderRow -> true
